...
| `console_check.py` (also on every deploy) | Renders seven representative pages and fails on any JavaScript error, unhandled rejection, failed subresource load, or CSP violation. Cloudflare's edge-injected analytics beacon is excluded, since it is absent when serving locally and a failure to load it here would say nothing about production. |
| `html_structure.py` (also on every deploy) | Parses every page and reports what a regex cannot see: unclosed or mismatched elements, duplicate ids, attribute values containing a quote or backslash, and `aria-labelledby`/`for`/`href="#..."` pointing at an id that does not exist. Written after qa.py passed four pages whose `<th>` tags held literal backslashes. |
